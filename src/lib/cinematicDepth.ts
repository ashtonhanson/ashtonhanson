...
/**
 * Home body copy — starts rotated clockwise, then sits on center as it scales.
 * Extra downward bias counters the lift from perspective + scale.
 */
export function homeCenteredBodyPath(): BezierPath {
  return {
    p0: { x: 8, y: 2.5, z: 0, scale: 1, rot: 10.5 },
    p1: { x: 4.2, y: 1.2, z: 0, scale: 1, rot: 5.4 },
    p2: { x: 1.1, y: 0.35, z: 0, scale: 1, rot: 1.2 },
...
export function sampleMobileBodyFillPose(
  ...
  const rest = 1;
  const peak = Math.max(coverScale, 2.6);
  const exitT = smootherstep(clamp((zoomT - 0.48) / 0.52, 0, 1));
  const z0 = ABOUT_INTRO.bodyStartZ;
  const z3 = ABOUT_INTRO.bodyPeakZ;
  return {
    x: 0,
