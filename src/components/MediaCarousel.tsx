...
declare module "react" {
  namespace JSX {
    interface IntrinsicElements {
      "ah-media-gallery-v24": React.DetailedHTMLProps<
...
export function MediaCarousel({ items, label = "Gallery" }: MediaCarouselProps) {
  ...
  useEffect(() => {
    ...
    apply();
    void customElements.whenDefined("ah-media-gallery-v24").then(apply);
    return () => {
      cancelled = true;
    };
  }, [items]);
  ...
  return (
    <>
      <ah-media-gallery-v24 ref={ref} label={label} className="block w-full" />
